...
@section('dashboard')
    <h2 class="text-2xl font-bold mb-4">Dashboard &gt; Kelola Kuisioner</h2>
    <div class="relative border border-gray-300 p-6 rounded-lg bg-gray-100 shadow-lg">
        ...
        <table class="w-full border-collapse bg-white shadow-lg rounded-lg">
            <thead>
                <tr>
                    ...
                    <th class="border-b-2 border-gray-300 py-2 px-4 text-left">Program Studi</th>
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td class="border-b border-gray-300 py-2 px-4">{{ $quisioner->jurusan }}</td>
                    <td class="border-b border-gray-300 py-2 px-4">{{ $quisioner->created_at }}</td>
                    <td class="border-b border-gray-300 py-2 px-4">
                        <form onsubmit="return confirm('Apakah anda yakin ingin menghapus program studi ini? ');" action="{{ route('dashboard.kuis.listkuis.destroy', $quisioner->id) }}" method="POST" class="d-inline">
                            <button class="px-2 py-1 bg-yellow-500 text-white rounded hover:bg-yellow-600">
                                <a href="{{ route('dashboard.kuis.editkuis', $quisioner->id) }}">Edit</a>
                            </button>
                            @csrf
                            @method('DELETE')
        ...
        <div class="text-right space-x-2 mt-4">
            ...
            <button class="px-4 py-2 bg-blue-500 text-white rounded hover:bg-blue-600">
                <a href="{{ route('dashboard.kuis.kelolakuis') }}">Tambah</a>
            </button>
        </div>
    </div>
@endsection
